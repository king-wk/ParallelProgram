...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < size; j++)
        {
            ...
            for (int k = kk; k < n; k++)
            {
                __m256 t1 = _mm256_set1_ps(subA[k][row]);
                for (int w = row + 1; w < row + 1 + remain; w++)
                    subA[k][w] = subA[k][w] - line[w] * subA[k][row];
                for (int w = row + 1 + remain; w < N; w += 8)
                {
                    __m256 t2 = _mm256_loadu_ps(&line[w]);
                    __m256 t3 = _mm256_loadu_ps(&subA[k][w]);
                    __m256 t4 = _mm256_mul_ps(t1, t2);
                    t4 = _mm256_sub_ps(t3, t4);
                    _mm256_storeu_ps(&subA[k][w], t4);
                }
                subA[k][row] = 0;
            }
        }
